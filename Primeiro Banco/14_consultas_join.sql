...
-- 4/Selecionar produtos e seus respetivos valores
...
-- 6/Obter o nome e o numero de telefone da cada cliente
...
-- 7/Obter o nome dos funcionarios e o nome do departamento en que trabalha
...
-- 10.Listar os produtos fornecidos por um fornecedor especifico
SELECT `prod_desc`
FROM `bd_caso_estudo_vendas`.`tb_prod`
WHERE `prod_forn_cod` = 1;

-- 11. Obter a soma total das compras realizadas por um cliente específico
SELECT `cli_nome`, SUM(`compra_qtd_cod`) AS `total_compras` -- as `nome_coluna` defina a coluna que ficara a soma
FROM `bd_caso_estudo_vendas`.`tb_cli`
JOIN `bd_caso_estudo_vendas`.`tb_compra`
ON `tb_cli`.`cli_cod` = `tb_compra`.`compra_cli_cod`
WHERE `cli_cod` =  2 -- Substituir conforme necessário
GROUP BY `cli_nome`;
...
SELECT `cli_nome`
FROM `bd_caso_estudo_vendas`.`tb_cli`
LEFT JOIN `bd_caso_estudo_vendas`.`tb_cli_tel` -- LEFT JOIN = Juntar a esquerda
ON `tb_cli`.`cli_cod` = `tb_cli_tel`.`tel_cli_cod`
...
select count(*) from tb_func; -- count serve para contar a qnt de linhas
...
LIMIT 10; -- Limita a quantidade de linhas
